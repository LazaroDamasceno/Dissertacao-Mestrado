...
Escolheram-se os índices de governo eletrônico remanescentes. A escolha foi inspirada em \cite{mitkiewicz2024transformaccao}. O autor, na sua participação no livro \textbf{Digitalização e tecnologias da informação e comunicação: oportunidades e desafios para o Brasil}\footnote{cf. \url{http://dx.doi.org/10.38116/9786556350660cap8}}, descreveu o histórico de governo eletrônico e digital no Brasil até a criação e implementação da plataforma e portal único do Governo Federal, o gov.br. \cite{mitkiewicz2024transformaccao} usou o GTMI, EGDI e o DGI para a avaliação das políticas de governo eletrônica e digital.

Será uma feita uma regressão polinomial para o EGDI, prevendo os valores do EDGI e seus sub-índices de 2028 até 2050, porque ele tem 12 edições (até 2025), enquanto os índices remanescentes tem duas edições (até 2025). Será Serão .
...
A composição do EGDI é demonstrada pela figura \ref{fig:egdi_sub-índices}.

\begin{figure}[H]
    \centering
    \caption{Sub-índices do EGDI}
    \includegraphics[width=1\linewidth]{figuras/egdi_componentes.png}
    \label{fig:egdi_sub-índices}
    \footnotesize{Fonte: \cite{ONU_EGDI_description}.}
\end{figure}

De maneira detalhada, a tabela \ref{tab:egdi_subindices_componentes} mostra os sub-índices do EGDI.

\begin{table}[H]
	\centering
	\caption{EGDI: sub-índices e sub-índices}
	\label{tab:egdi_sub-índices_subindices}
	\begin{tabular}{|p{5.5cm}|l|}
		\hline
		\textbf{sub-índices} & \textbf{Componentes} \\
		\hline
...
Considerando as informações anteriores, as figuras \ref{fig:mapa_coropletico_paises_egdi}, \ref{fig:mapa_coropletico_paises_epi}, \ref{fig:mapa_coropletico_paises_hci}, \ref{fig:mapa_coropletico_paises_osi} e \ref{fig:mapa_coropletico_paises_tii} contêm mapas coropléticos que mostram o EGDI, seus sub-índices e o EPI pelo mundo.
...
\begin{figure}[H]
	\centering
	\caption{Coeficientes de correlação entre EGDI, seus sub-índices e o EPI com o índice de democracia eleitoral}
...
\begin{figure}[H]
	\centering
	\caption{Coeficientes de correlação entre EGDI, seus sub-índices e o EPI com o PIB \textit{per capita} PPC}
...
\begin{figure}[H]
	\centering
	\caption{Coeficientes de correlação entre EGDI, seus sub-índices e o EPI com os gastos públicos como percentual do PIB}
...
Descobriu-se que o EGDI, seus sub-índices e o EPI quando comparados com o PIB \textit{per capita} PPC têm coeficientes de correlação acima de 0,6, o que indica que há correlação forte entre as variáveis. Do resultado, deduz-se que há uma tendência do crescimento do PIB de afetar positivamente o EGDI, que seguiria a tendência de crescimento.

Já a relação entre o EGDI, seus sub-índices e o EPI quando comparados com os gastos públicos como percentual do PIB indica uma fraca correlação, que, no máximo, passou um pouco de 0,3. Disso, extrai-se o entendimento de que as variáveis são independentes entre si.
...
\begin{figure}[H]
	\centering
	\caption{EGDI, seus sub-índices e do EPI no Brasil (2003-2005, 2008-2024 bienalmente)}
...
Nota-se como o TII foi o componente que mais cresceu de forma contínua; já o OSI teve a maior queda, porém o componente que atingiu o nível mais alto de todos os sub-índices. O HCI foi o único componente em teve queda no valor. Por fim, o EGDI teve unas quedas, porém voltou a subir.
