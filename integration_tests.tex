...
The figure \ref{fig:medical_slot_cancellation_sequence_diagram} contains the sequence diagram for completing a medical slot.

\begin{figure}[H]
	\centering
	\caption{Medical Slot Cancellation Flow}	
	\includegraphics[width=1\linewidth]{figures/medical_slot_cancellation_sequence_diagram.png}
	\footnotesize Source: Author's creation.
	\label{fig:medical_slot_cancellation_sequence_diagram}
\end{figure}

The client, whose behavior is described in the \hyperref[subsection:http_semantics]{subsection HTTP Semantics}, requests a resource. It uses the \hyperref[appendix:glossary]{URI} \textit{/api/v2/medical\_slots/\underline{license\_number}/\underline{state}/\underline{medical\_slot\_id}/cancellation}. The \hyperref[appendix:glossary]{URI} follows the pattern established  in the table \ref{tab:http-server-schemes}.

The path variables \textit{license\_number}, \textit{state}, and \textit{medical\_slot\_id} are passed, respectively, to the \hyperref[appendix:glossary]{URI} \textit{/api/v2/medical\_slots/\underline{license\_number}/\underline{state}/\underline{medical\_slot\_id}/cancellation} as arguments. The service cancels the sought \textbf{MedicalSlot}, then persists it.

As exposed by the figure \ref{fig:medical_slot_cancellation_sequence_diagram}, the sought \textbf{Doctor} is retrieved by the utility class \textbf{DoctorFinder}. If the outcome is null, the \hyperref[tab:summary_http_status_codes]{status code 404} is returned  with the exception \textit{NonExistentDoctorException}; otherwise, the sought \textbf{Doctor} is returned. 
Although, when the provided \textit{medical\_slot\_id} is not associated with any existing \textbf{MedicalSlot}, the \hyperref[tab:summary_http_status_codes]{status code 404} is returned with the exception \textit{NonExistentMedicalSlotException}. 

Additionally, when sought \textbf{MedicalSlot} is completed, the \hyperref[tab:summary_http_status_codes]{status code 409} is returned with the exception \textit{ImmutabletMedicalSlotException}. 

As the default behavior, the sought \textbf{MedicalSlot} is marked as canceled, persisted, and wrapped in the \hyperref[tab:summary_http_status_codes]{status code 204}.

\subsubsection{Completion Process}
\label{subsection:medical_slot_completion}

The figure \ref{fig:medical_slot_completion_sequence_diagram} contains the sequence diagram for completing a medical slot.

\begin{figure}[H]
	\centering
	\caption{Medical Slot Completion Sequence Diagram}	
	\includegraphics[width=1\linewidth]{figures/medical_slot_completion_sequence_diagram.png}
	\footnotesize Source: Author's creation.
	\label{fig:medical_slot_completion_sequence_diagram}
\end{figure}

The client, whose behavior is described in the \hyperref[subsection:http_semantics]{subsection HTTP Semantics}, requests a resource. It uses the \hyperref[appendix:glossary]{URI} \textit{/api/v2/medical\_slots/\underline{license\_number}/\underline{state}/\underline{medical\_slot\_id}/completion}. The \hyperref[appendix:glossary]{URI} follows the pattern established  in the table \ref{tab:http-server-schemes}.

The path variables \textit{license\_number}, \textit{state}, and \textit{medical\_slot\_id} are passed, respectively, to the \hyperref[appendix:glossary]{URI} \textit{/api/v2/medical\_slots/\underline{license\_number}/\underline{state}/\underline{medical\_slot\_id}/completion} as arguments. The service cancels the sought \textbf{MedicalSlot}, then persists it.

As exposed by the figure \ref{fig:medical_slot_completion_sequence_diagram}, the sought \textbf{Doctor} is retrieved by the utility class \textbf{DoctorFinder}. If the outcome is null, the \hyperref[tab:summary_http_status_codes]{status code 404} is returned  with the exception \textit{NonExistentDoctorException}; otherwise, the sought \textbf{Doctor} is returned. 
Although, when the provided \textit{medical\_slot\_id} is not associated with any existing \textbf{MedicalSlot}, the \hyperref[tab:summary_http_status_codes]{status code 404} is returned with the exception \textit{NonExistentMedicalSlotException}. 

Additionally, when sought \textbf{MedicalSlot} is completed,canceled the \hyperref[tab:summary_http_status_codes]{status code 409} is returned with the exception \textit{ImmutabletMedicalSlotException}. 

As the default behavior, the sought \textbf{MedicalSlot} is marked as completed, persisted, and wrapped in the \hyperref[tab:summary_http_status_codes]{status code 204}.
